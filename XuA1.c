...
int Forge(pairing_t pairing, Vehicle vehicles[40],    element_t S,     const char **m,         element_t P,    element_t Y){
    ...
    element_set0(P3_agg);
    element_set0(P4_agg);
    element_random(vehicles[0].PID1i);// 🔥 V_0's pseudonym is randomly generated by the adversary.
    element_random(vehicles[0].PID2i);
    element_random(vehicles[0].xi);
    element_mul_zn(vehicles[0].Xi, P, vehicles[0].xi);// 🔥 V_0's public key is randomly generated by the adversary.
    H1(h10, vehicles[0].PID1i, vehicles[0].PID2i, vehicles[0].Xi);
    element_mul_zn(Temp1_G, Y, h10);
    element_add(Temp1_G, Temp1_G, vehicles[0].PID1i);
    element_neg(Temp1_G, Temp1_G);
    for(int i=1; i<40; i++){
       element_random(ri);
    ...
    element_add(h30, h40, P4_agg);// 🔥 Here we force Σh_3i=h_40. It can be realized by TestH.c
    element_random(r0);
    element_mul_zn(vehicles[0].U, P, r0);
    element_add(vehicles[0].U, vehicles[0].U, Temp1_G);
    element_mul(S, h30, r0);
    element_mul(S1, h50, vehicles[0].xi);
    element_add(S, S, S1);      
...
{   Vehicle V[40];
    ...
    }//🔥 Only 39 legitimate vehicles generate their key pairs.
    
    clock_gettime(CLOCK_MONOTONIC, &start1);
    for(int i=1; i<40; i++){
    Extraction(V[i].ai, V[i].PID1i, V[i].PID2i, V[i].Xi, y, z, P, Vi[i], pairing); 
    }//🔥 The TA only generates 39 partial private keys.
    ...
    }//Verify 39 partial private keys.
    clock_gettime(CLOCK_MONOTONIC, &end12); 
    elapsed12 = ((end12.tv_sec - start12.tv_sec) + (end12.tv_nsec - start12.tv_nsec) / 1e9)/40;  
  
     
    Forge(pairing, V, S, msgs,   P, Y);//🔥 All 39 vehicles collude to masquerade as a fake vehicle. 
