...
void Forge(pairing_t pairing, Vehicle vehicles[40], element_t S, const char **m, const char *t, element_t P, element_t Y){
    ...
    for(int i=1; i<40; i++){
       ...
       element_random(vehicles[i].U);       
        H1(h1i, vehicles[i].PID1i, vehicles[i].PID2i, vehicles[i].Xi);        
        H3(h3i, vehicles[i].PID1i, vehicles[i].PID2i, vehicles[i].U, mi, t); 
        element_mul_zn(temp_G1, vehicles[i].Xi, h3i);

        element_add(temp_Zr, temp_Zr, h1i);
        element_add(P1_agg, P1_agg, vehicles[i].U);
        element_add(P2_agg, P2_agg, vehicles[i]. PID1i);
...
}/*the adversary only has vehicle[0].ai and vehicle[0].xi. It cannot modify Xi or Ai*/
...
int aggregate_verify(pairing_t pairing, Vehicle vehicles[40],      element_t S,     const char **m,        const char *t,            element_t P,    element_t Y) {
    ...
    for (int i = 0; i < 40; i++) {
        ...
        H1(h1i, PID1i, PID2i, Xi);        // α_i = H1(PID_i)
        H3(h3i, PID1i, PID2i, vehicles[i].U, mi, t); // 4. H3(PID_i, m[i], t)
        element_mul_zn(temp_G1, Xi, h3i);//H3·PID1_i
        element_mul_zn(temp_G2, Y, h1i);
        element_add(P1_agg, P1_agg, Ti);
        element_add(P2_agg, P2_agg, PID1i);
        element_add(P3_agg, P3_agg, temp_G1); // 5. H_agg = Σ H3·PID1_i
...
{   Vehicle V[40];
   ...
   for(int i=0; i<40; i++){
   ...
   }// 🔥 All 40 public keys are generated by legitimate vehicles.
    
    clock_gettime(CLOCK_MONOTONIC, &start1);
    for(int i=0; i<40; i++){
        Extraction(V[i].ai, V[i].PID1i, V[i].PID2i, V[i].Xi, y, z, P, Vi[i], pairing); 
    }// 🔥 The TA generate all 40 partial private keys.
    ...
    }// 🔥 Verify all 40 partial private keys.
  ...
  elapsed12 = ((end12.tv_sec - start12.tv_sec) + (end12.tv_nsec - start12.tv_nsec) / 1e9)/40;   
    ...
    Forge(pairing, V, S, msgs, t, P, Y);// 🔥 the adversary cannot modify public keys or identities/pseudonyms.
      
   
    int result=aggregate_verify(pairing, V, S, msgs,   t, P, Y);
    if(result)
