...
int H3(element_t out, element_t PID1i, element_t PID2i, element_t B, const char *data){
    char buf[1256];
    int pos=0;
    pos += element_to_bytes(buf + pos, PID1i);
    pos += element_to_bytes(buf + pos, PID2i);
...
int Forge(pairing_t pairing, Vehicle vehicles[40],    element_t S,     const char **m,         element_t P,    element_t Y){
    element_t h0i, h3i, ri, Temp_Zr, P1_agg, P2_agg, P3_agg, P4_agg, r0;
    element_t temp_G1, temp_G2, temp_G3;
    element_init_Zr(h0i, pairing);
    ...
    element_mul_zn(temp_G3, Y, h0i);      
    element_add(temp_G3, temp_G3, vehicles[0].PID1i);   
    element_add(temp_G3, temp_G3, vehicles[0].Xi);    
    element_neg(temp_G2, temp_G3);   
    for(int i=1; i<40; i++){
       element_random(ri);
       element_mul_zn(temp_G1, P, ri);
       element_add(vehicles[i].U, temp_G1, temp_G2);
       const char *mi = m[i]; 
       H3(h3i, vehicles[i].PID1i, vehicles[i].PID2i, vehicles[i].U, mi);
       element_add(P4_agg, P4_agg, h3i);   
       element_mul(Temp_Zr, h3i, ri);
       element_add(P1_agg, P1_agg, Temp_Zr);
       element_add(P2_agg, P2_agg, vehicles[i].ai);
       element_add(P3_agg, P3_agg, vehicles[i].xi);  
    }
    element_neg(P4_agg, P4_agg);
    element_set1(h3i);
    element_add(h3i, h3i, P4_agg);//🔥 Here we force Σh_3i=1. It can be realized by Test1.c
    element_random(r0);   
    element_mul(S, h3i, r0);
    element_mul_zn(vehicles[0].U, P, r0);
    element_add(vehicles[0].U, temp_G2, vehicles[0].U);
...
int aggregate_verify(pairing_t pairing, Vehicle vehicles[40],      element_t S,     const char **m,            element_t P,    element_t Y) {
    int result = 0;
    element_t  P1_agg, P2_agg, P3_agg, P4_agg, Ti, Xi, temp_G1, temp_G2, temp_G3, temp_G4;
    element_t  h1i, h3i, h2i, tt;
    element_t e_left, e_right, PID1i, PID2i;
    element_init_Zr(h2i, pairing);
    ...
    for (int i = 1; i < 40; i++) {
        ...
        H3(h3i, PID1i, PID2i, vehicles[i].U, mi);     
        ...
        element_add(P3_agg, P3_agg, Xi); 
        element_add(P4_agg, P4_agg, temp_G2);
        element_add(tt, tt, h3i);
    }     
    element_neg(tt, tt);
    element_set1(h3i);
    element_add(h3i, tt, h3i);
    element_mul_zn(temp_G3, vehicles[0].U, h3i);
    H1(h1i, vehicles[0].PID1i, vehicles[0].PID2i, vehicles[0].Xi);  
    element_mul_zn(temp_G4, Y, h1i);
    element_mul_zn(e_left, P, S);   //S*P
    element_set0(tt);
    for(int i=1; i<40; i++){
       const char *mi = m[i]; 
       H3(h2i, vehicles[i].PID1i, vehicles[i].PID2i, vehicles[i].U, mi);
       element_add(tt, tt, h2i);
        }
    element_add(h3i, h3i, tt);
    ...
    element_add(e_right, vehicles[0].PID1i, e_right);
    element_add(e_right, vehicles[0].Xi, e_right);
